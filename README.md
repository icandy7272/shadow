...
`drill` 挖掉被弱读的功能词让你听着填。**听出来的直接写；靠上下文猜的，在词后加个
问号**（如 `up?`）——功能词恰恰是最容易从语法推断出来的一类词（`put me ___ for
adoption` 闭着眼也能填 `up`），不把「听到」和「推断」分开，填空就测不出听力。
**填空正确率是唯一客观测量听力的指标**——
...
**默认严格一句一个。** 太长的句子（超过 6 秒）会在最大停顿处再断；只有单词句会并进
下一句，避免出现只有一个词的单元。

想要更长的单元就抬高下限，短句会被并进下一句：

```bash
uv run shadow units 2 --min-sec 1.5
```

实测 0.7 秒 / 2 词的短句仍有 25-54 个浊音帧，高于音高分析所需的 20 帧下限，
所以短单元的反馈依然算得出来。

注意：**改了切分规则，单元序号指的就是不同内容了**。所以每轮练习都会把当时练的
那句话原文存进库，`progress` 显示的是句子而不是序号，历史记录不会因此失效。

录音命令的试听选项会在录音前自动放若干遍。实测「听几遍 → 同步跟读 → 独立说一遍」
