...
**网页版复用时漏掉的一步（2026-09-10）：**

| 处 | 问题 | 修正 |
|---|---|---|
| `web/app.py` | `_unit_audio` 裁出单元音频，却把库里的**绝对**时间戳原样交给评估链路。词标在素材的第 40 秒，音频只有 1.5 秒，`word_contour` 一帧都取不到，图 2 的原声轮廓整条变平（全是 0），升降调判断跟着全部失效——网页版从上线起给出的句尾降调反馈都是错的 | 抽出 `_unit_reference`，与命令行的 `_segment_reference` 一样把时间戳平移到以裁剪起点为 0；`UNIT_PAD_SEC` 挪进 `config`，两边不会再各裁各的 |

教训与 Task 15 同源：命令行把这一步写在 `_segment_reference` 的 docstring 里，
网页版复用了 `split_into_units` 却没复用这个平移，两条路各自演化。
`review.py` 当初就是为了避免这种分叉而抽出来的，但抽的是**评估**，
取原声这一段没抽，于是分叉从这里冒了出来。
